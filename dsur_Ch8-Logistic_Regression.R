...
# In our regression, we want:
# Predictor: Baseline/0 = "No Treatment", 1 = "Treatment"
# Outcome: odds = P(cured)/P(not cured), odds ratio = Change in proportion in odds (of cured) when we apply treatment
# 
# So we switch "Not Cured" and "No Treatment" to become baseline level
...
############
# Carry out a hierarchical regression

# glm: generalized linear model
#
# family: a description of the error distribution and link function to be used in the model
# If we were doing an ordinary regression we could set this option to Gaussian (i.e. normal distribution).
# Logistic regression is based on a binomial distribution, so we set family to binomial().
...
summary(eelModel.1)
# For the null model, (deviance=) −2LL = 154.08, but when Intervention has been included this value has been reduced to 144.16. 
# This reduction tells us that the model is better at predicting whether someone was cured than it was before 
# Intervention was added.
#
# Note: 
# deviance is like the "residual sum of square" in linear regression.  So the smaller the better.
# deviance has a chi-square distribution
#
# The coefficient:
# Intervention was a significant predictor of "being cured", b = 1.23, z = 3.07, p < .002.
...
# "Null.deviance - Deviance" has a chi-square distribution with degree of freedom = "chidf" as calculated below.
modelChi <- eelModel.1$null.deviance - eelModel.1$deviance
chidf <- eelModel.1$df.null - eelModel.1$df.residual

# To calculate the probability associated with this chi-square statistic
chisq.prob <- 1 - pchisq(modelChi, chidf)
modelChi; chidf; chisq.prob
# chisq.prob = 0.002 < 0.05
# So we can reject the null hypothesis that the model is not better than chance alone (the baseline null model) at 
# predicting the outcome.
#
# We can report that including Intervention produced a significant improvement in the fit of the model, χ2(1) = 9.93, p = .002.

# Calculate R^2 based on different methods:
...
# All these three different R^2 can be used as effect size measures for the model.

#
# To calculate the change in odds that results from a unit change in the predictor for this example, we must first calculate 
# the odds of a patient being cured given that they didn’t have the intervention. We then calculate the odds of a patient 
# being cured given that they did have the intervention. Finally, we calculate the proportionate change in these two odds.

# Compute odds ratio using the easy way:
exp(eelModel.1$coefficients)
# Odds ratio = 3.42, we can say that the odds of a patient who is treated being cured are 3.42 times higher than those of 
# a patient who is not treated.

# Confidence interval of odd ratios, to see what the range of odds ratio in the population.
exp(confint(eelModel.1))
# Based on the result, we can be fairly confident that the population value of the odds ratio lies between 1.58 and 7.63.
#
# The important thing about this confidence interval is that it doesn’t cross 1 (the values at each end of the interval are 
# greater than 1). This is important because values greater than 1 mean that as the predictor variable increases, so do the 
# odds of (in this case) being cured. Values less than 1 mean the opposite. 
#
# The fact that both the lower and upper limits of our confidence interval are above 1 gives us confidence that the direction
# of the relationship that we have observed is true in the population (i.e., it’s likely that having an intervention compared
# to not increases the odds of being cured). If the lower limit had been below 1 then it would tell us that there is a chance
# that in the population the direction of the relationship is the opposite to what we have observed. This would mean that we
# could not trust that our intervention increases the odds of being cured.


##########
# compare model1 and model 2 by finding the difference in deviances

# Way #1: Manually calculate the difference in deviance
...
eelData$predicted.probabilities<-fitted(eelModel.1) # fitted() get you the fitted (expected probabilities) from a model
eelData$standardized.residuals<-rstandard(eelModel.1)
eelData$studentized.residuals<-rstudent(eelModel.1)

# dfbeta = The difference between a parameter estimated using all cases and estimated when one case is excluded is known.
eelData$dfbeta<-dfbeta(eelModel.1) 

# dffits = difference between the predicted value for a case when the model is calculated including that case and when 
# the model is calculated excluding that case.
eelData$dffit<-dffits(eelModel.1)

# leverage = a measure of how sensitive the regression is to a given case.  0 = no influence, 1 = complete influence
eelData $leverage<-hatvalues(eelModel.1)

head(eelData[, c("Cured", "Intervention", "Duration", "predicted.probabilities")])

# We will examine the basic residual statistic (leverage, studentized residual, dfbeta), for two main reasons:
# (1) isolate points for which the model fits poorly
# (2) isolate points that exert an undue influence on the model.
#
